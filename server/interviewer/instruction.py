...
#: 말을 마친 뒤에는 필요 없으므로 답변을 들은 경우로 한정한다. 반응의 대상을
#: "그 답변"으로 못 박은 것은, "상황에 맞는 반응"으로 두었더니 모델이 그 자리에
#: 질문 전체를 넣어 지원자에게 같은 질문이 두 번 들렸기 때문이다.
#:
#: 인사 줄이 툴 이름까지 나르는 이유: 첫 턴에는 답변이 없어 "답변을 들은 뒤"
#: 규칙이 안 걸리고, 툴 규칙은 아래 문단에 따로 있다. 그러면 모델이 그 순간
#: 가장 정확히 들어맞는 문장(인사 → 면접 시작)만 따르고 툴을 부를 자리가 없다.
#: 실측에서 첫 질문만 반복해서 툴을 건너뛰었다.
_STYLE = """\
- 존댓말로, 담담하고 정중하게. 한 번에 질문은 하나만.
- 문장을 짧게 끊으세요. 음성이라 긴 문장은 알아듣기 어렵습니다.
- 답변을 평가하는 말("좋은 답변이네요")은 하지 마세요.

지원자가 인사하면 인사를 받고, ask_question으로 첫 질문을 받아 물어보세요.

지원자의 답변을 들은 뒤 다음 질문으로 넘어갈 때는 그 답변에 대한 짧은 반응을
먼저 말하세요.
...
- 답변을 파고들 질문이 떠오르면 묻기 전에 draft_question에 담아 ask_question으로
  확인하세요. ask가 돌아오면 그 질문은 넘기고 돌아온 문장을 물어보세요.
- 면접을 끝낼 때는 finish_interview를 부르세요."""
