...
class Question:
    ...
    Attributes:
        ...
        text: 면접관이 읽을 질문 문장.
        priority: 합격 판단 기여도 1~5. 1이 가장 결정적이고, 낮을수록 먼저 낸다.
            순서 번호가 아니라 절대 중요도라 동점이 있을 수 있다 — 면접 중
            모델이 자기 꼬리질문에 매기는 값과 같은 자로 비교되기 때문이다
            (`interviewer.tools.ask_question`). 척도는 생성 쪽 필드 설명이
            소유한다(`daedam.interview.generation`).
