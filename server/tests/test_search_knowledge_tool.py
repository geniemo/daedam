...
def test_리포트에서_관련_정보를_찾는다() -> None:
    ...
    result = search_knowledge(tool_context=ToolContextStub(STATE), query="정산 주기")
...
def test_source로_지원서만_좁힌다() -> None:
    result = search_knowledge(
        tool_context=ToolContextStub(STATE), query="정산", source="application"
    )
...
def test_무관한_질의는_빈_결과와_note() -> None:
    ...
    result = search_knowledge(tool_context=ToolContextStub(STATE), query="등산 코스 추천")
    assert result["results"] == []
...
def test_state가_없으면_스모크_코퍼스에서_찾는다() -> None:
    ...
    result = search_knowledge(tool_context=ToolContextStub(), query="배차 자동화")
    assert result["results"]
...
def test_선언에_query는_필수_source는_enum이다() -> None:
    ...
    asyncio.run(
        FunctionTool(search_knowledge).process_llm_request(
            tool_context=ToolContextStub(), llm_request=request
        )
    )
