...
def _row(at: float, cell: int, impression: str, ratio: float = 1.0) -> dict:
    return {"at": at, "cell": cell, "impression": impression, "ratio": ratio}
...
def test_전체_비율을_낸다() -> None:
    rows = [_row(i, CENTER_CELL, "focused") for i in range(6)]
    rows += [_row(i, 1, "tense", 5.0) for i in range(6, 10)]
    ...
    assert got["impressions"]["focused"] == 0.6
    assert got["impressions"]["tense"] == 0.4
    assert sum(got["cells"]) == 1.0


def test_답변_구간으로_자른다() -> None:
    """같은 면접에서도 답변마다 다르게 나와야 쓸모가 있다."""
    rows = [_row(i, CENTER_CELL, "focused") for i in range(0, 10)]
    rows += [_row(i, 7, "flustered", 6.0) for i in range(10, 20)]
    got = analyze(
        {"seconds": rows},
    ...
    assert first["impressions"]["focused"] == 1.0
    assert second["seconds"] == 10 and second["steady"] == 0.0
    assert second["impressions"]["flustered"] == 1.0
...
def test_답변_구간에_담긴_줄이_없으면_0이다() -> None:
    ...
    got = analyze(
        {"seconds": [_row(1, CENTER_CELL, "focused")]},
...
def test_모르는_칸과_인상은_세지_않는다() -> None:
    """화면과 서버의 분류가 어긋나도 비율이 깨지지 않아야 한다."""
    rows = [
        _row(0, CENTER_CELL, "focused"),
        _row(1, 99, "확인불가"),          # 둘 다 모르는 값
    ]
    got = analyze({"seconds": rows})
    assert got["seconds"] == 2
    assert got["steady"] == 0.5                       # 아는 것만 셌다
    assert sum(got["impressions"].values()) == 0.5
