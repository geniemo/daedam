...
### Responsive
- 분기는 **하나**: 720px 미만이 좁은 화면(`useNarrow`, 코드에서는 `md:`). 중간 단계를 만들지 않는다.
- 좁은 화면 규칙: 페이지 좌우 여백 32 → 16~20, 2~3열 격자는 1열(지표만 2열), 무대 띠·리포트 머리·다음 면접 카드는 세로로 쌓이고 버튼은 가로 100%, 제목은 한 단계 작게(44→30, 27→23, 25→21, 56→44), 헤더는 이름 글자를 숨기고 아바타만.
- 면접 무대(<600): 셀프뷰 132×99(우상단 16/8), 거울 PiP 64, 구체 최소 120·최대 폭의 50%, 질문 18px·폭 100%−32, 여백 16. 셀프뷰 동작 링크는 세로로 쌓고 구분점을 뺀다.
- 카드 격자는 `minmax(min(340px, 100%), 1fr)` — 340보다 좁은 화면에서도 한 열이 넘치지 않는다. 카메라 미리 보기는 `aspect-ratio: 4/3`, 최대 320.
- 킷 미리 보기: 페이지 주소 뒤에 `?w=390`을 붙이면 그 폭으로 렌더한다.
