      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {camera === 'on' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, paddingLeft: 24 }}>
            <div style={{ width: 320, height: 240, borderRadius: 'var(--radius-card)', border: '1px solid var(--color-line)', background: 'var(--color-surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11.5, color: 'var(--color-faintest)' }}>웹캠 미리보기 · 거울 반전</div>
...
function Threshold({ card, ready, short, credits, onStart, stages = true, stagesAlign = 'center' }) {
  ...
  return (
    <div data-threshold="true" style={{ position: 'relative', overflow: 'hidden', borderRadius: 'var(--radius-card)', background: 'var(--stage-bg)', color: 'var(--stage-ink-warm)', marginBottom: 16, minHeight: 300, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'relative', flex: 1, display: 'flex', alignItems: 'center', gap: 28, padding: '6px 32px 28px' }}>
      ...
      {stages && (
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: stagesAlign === 'center' ? 'center' : 'flex-start', gap: stagesAlign === 'center' ? 0 : 0, borderTop: '1px solid rgba(255,255,255,.08)', padding: '12px 22px' }}>
          {STAGE_NAMES.map((n, i) => (
            <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 8, flex: stagesAlign === 'center' ? 'none' : 1, minWidth: 0 }}>
              ...
              {i < STAGE_NAMES.length - 1 && <span style={{ flex: stagesAlign === 'center' ? 'none' : 1, width: stagesAlign === 'center' ? 28 : undefined, height: 1, background: 'rgba(255,255,255,.08)', margin: '0 14px' }} />}
            </div>
          ))}
        </div>
      )}
...
function Ready({ card, credits, onBack, onReview, onStart, threshold = true, stages = true, stagesAlign = 'center' }) {
  ...
  return (
    <main style={{ maxWidth: 'var(--container-report)', margin: '0 auto', padding: '44px 32px 80px', animation: 'dm-fade .3s ease' }}>
      ...
      <h1 style={{ margin: 0, fontSize: 27, fontWeight: 700, letterSpacing: '-.03em' }}>{card.company} · {card.role}</h1>
      <div style={{ height: 26 }} />
      ...
      <Card style={{ marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', borderTop: '1px solid var(--color-hair)', paddingTop: 14 }}>
