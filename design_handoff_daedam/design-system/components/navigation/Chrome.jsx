...
import { useNarrow } from '../core/useViewport.jsx';
...
export function Chrome({ name = '지원자', credits, onHome, onAccount, onCredits, avatarUrl }) {
  ...
  return (
    <header style={{ position: 'sticky', top: 0, zIndex: 40, height: 64, background: 'var(--header-bg)', backdropFilter: 'blur(8px)', borderBottom: '1px solid var(--color-line)' }}>
      <div style={{ maxWidth: 'var(--container-home)', margin: '0 auto', height: '100%', padding: narrow ? '0 16px' : '0 32px', display: 'flex', alignItems: 'center', gap: narrow ? 16 : 32, boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {!narrow && <a onClick={onAccount} style={{ fontSize: 13, color: 'var(--color-muted)', cursor: 'pointer' }}>{name}</a>}
